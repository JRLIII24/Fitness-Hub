...
export default function WorkoutCalendarPage() {
  ...
  const [templateSession, setTemplateSession] = useState<SessionItem | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data, error } = await supabase
        ...
        .select(
          "id,name,started_at,completed_at,duration_seconds,workout_templates(name),workout_sets(set_number,reps,weight_kg,exercise_id,exercises(id,name,muscle_group))"
        )
        .eq("status", "completed")
  ...
  async function handleSaveAsTemplate(name: string, categories: string[]) {
    ...
    for (const set of templateSession.workout_sets) {
    ...
    const { data: template, error: templateError } = await supabase
      ...
      .insert({
        user_id: user.id,
        name,
        description: `Created from ${templateSession.name} on ${format(new Date(templateSession.started_at), "MMM d, yyyy")}`,
        primary_muscle_group: categories.length > 0 ? categories[0] : null,
      })
      .select("id")
      .single();
    ...
    const exerciseRows = exerciseOrder.map((exerciseId, index) => ({
      template_id: template.id,
      exercise_id: exerciseId,
      sort_order: index,
      target_sets: (setsByExercise.get(exerciseId) ?? []).length,
      target_reps: null as string | null,
  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-6 px-4 pt-6 pb-28 md:px-6 lg:px-10">
      <TrainSubNav />
      <div>
        <h1 className="text-xl font-bold text-foreground sm:text-2xl">Calendar</h1>
        <p className="mt-1 text-[13px] text-muted-foreground">
          View your workout history by date.
        </p>
      </div>
      ...
      <SaveTemplateDialog
        ...
        defaultName={templateSession?.name ?? ""}
        defaultCategories={
          templateSession
            ? [...new Set(
                templateSession.workout_sets
                  .map((s) => s.exercises?.muscle_group?.toLowerCase())
