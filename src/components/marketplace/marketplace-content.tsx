...
export function MarketplaceContent() {
  ...
  // ── load templates ────────────────────────────────────────────────────────
  const loadTemplates = useCallback(async (pg: number, append = false) => {
    try {
  ...
  // reset + reload templates when filters/sort/tab change
  useEffect(() => {
    setPage(1);
    setLoading(true);
    loadTemplates(1).finally(() => setLoading(false));
  }, [loadTemplates]);
